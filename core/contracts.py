# -*- coding: utf-8 -*-
"""
contracts.py - 合同/项目管理核心模块 / Contract & Project Management Core Module

本模块是整个 ERP 系统的核心业务逻辑层，涵盖合同全生命周期管理：
This module is the core business logic layer of the ERP system, covering the complete contract lifecycle:

=== Contract CRUD / 合同基础操作 ===
- list_contracts: 项目列表（支持多条件筛选）/ Project list with filtering
- new_contract: 新建项目/合同 / Create new contract
- edit_contract: 编辑合同信息 / Edit contract information
- contract_overview: 项目总览页面 / Project overview dashboard
- set_status_note: 设置状态描述 / Set status note
- set_planned_delivery: 设置计划交付日期 / Set planned delivery date

=== Task Management / 任务管理 ===
- manage_tasks: 管理项目任务 / Manage project tasks
- change_task_status: 任务状态变更 / Change task status
- tasks_by_department: 按部门查看任务 / View tasks by department
- tasks_by_person: 按人员查看任务 / View tasks by person

=== Procurement / 采购管理 ===
- manage_procurements: 管理采购清单 / Manage procurement list
- delete_procurement: 删除采购项 / Delete procurement item

=== Acceptance / 验收管理 ===
- manage_acceptances: 管理验收记录 / Manage acceptance records
- set_acceptance_status: 更新验收状态 / Update acceptance status

=== Feedback / 售后反馈 ===
- manage_feedbacks: 管理客户反馈 / Manage customer feedback
- resolve_feedback: 标记反馈已解决 / Mark feedback as resolved

=== File Management / 文件管理 ===
- manage_files: 项目文件管理 / Project file management
- upload_file: 文件上传 / File upload
- download_file: 文件下载 / File download
- preview_file: 文件预览 / File preview
- export_files_csv: 导出文件列表 / Export file list

=== Leaders & Sales / 负责人与销售 ===
- manage_leaders: 部门负责人管理 / Department leader management
- manage_sales: 销售信息管理 / Sales information management

=== Notifications / 通知功能 ===
- notify_contract: 手动发送通知 / Manual notification sending
...
def contract_overview(contract_id):
    ...
    # 甘特图数据
    tasks_all = Task.query.filter_by(contract_id=contract.id).order_by(Task.start_date.asc()).all()
    gantt_tasks = []
    for t in tasks_all:
        if t.start_date and t.end_date:
            gantt_tasks.append({
                ...
                "progress": 100 if t.status == '已完成' else (50 if t.status == '进行中' else 0),
                "dependencies": ""
            })
...
def set_planned_delivery(contract_id: int):
    ...
    allowed_usernames = ['张三', '李四', 'wangwu']
...
def preview_converted_file_raw(contract_id, file_id):
    ...
    # ---- 第一步：走 preview_service 的统一逻辑（优先用）----
