# -*- coding: utf-8 -*-
"""
file_service.py - 文件管理服务模块 / File Management Service Module

本模块封装项目文件的完整生命周期管理：
This module encapsulates the complete lifecycle management of project files:

- save_uploaded_file: 单文件上传 / Single file upload
- save_multiple_files: 批量文件上传 / Batch file upload
- get_file_for_download: 下载权限校验 / Download permission check
- soft_delete_file: 软删除 / Soft delete
- restore_file: 文件恢复 / File restore
- set_public: 公开/私有控制 / Public/private control

Features / 功能特点:
- 基于角色的上传权限控制 / Role-based upload permission control
- 结构化文件命名规则 / Structured file naming rules
- 项目编号分目录存储 / Project code based directory storage
- 完整的操作日志记录 / Complete operation logging
