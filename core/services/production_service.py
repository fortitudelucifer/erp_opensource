# -*- coding: utf-8 -*-
"""
production_service.py - 生产任务服务模块 / Production Task Service Module

本模块封装生产任务的业务逻辑：
This module encapsulates the business logic for production tasks:

- create_task: 创建任务 / Create task
- change_status: 状态流转 / Status transition
- start_task: 开始任务 / Start task
- complete_task: 完成任务 / Complete task
- mark_waiting_qc: 标记待质检 / Mark as waiting for QC
- pause_task: 暂停任务 / Pause task

Task Status Flow / 任务状态流转:
未开始 → 进行中 → 待质检 → 已完成
Not Started → In Progress → Waiting QC → Completed
