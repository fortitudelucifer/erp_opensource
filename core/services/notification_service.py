# -*- coding: utf-8 -*-
"""
notification_service.py - 多渠道通知服务模块 / Multi-Channel Notification Service Module

本模块提供统一的通知服务接口，支持多种通知渠道：
This module provides a unified notification service interface supporting multiple channels:

Channels / 通知渠道:
- email: 邮件通知 (SMTP)
- sms: 短信通知 (预留接口)
- ding: 钉钉群机器人 / DingTalk bot
- wechat_corp: 企业微信群机器人 / WeCom bot

Architecture / 架构设计:
- NotificationService: 协议接口 / Protocol interface
- DummyNotificationService: 开发测试用占位实现 / Placeholder for dev/test
- EmailNotificationService: SMTP 邮件实现 / SMTP email implementation
- DingTalkRobotNotificationService: 钉钉机器人实现 / DingTalk bot implementation
- WeComRobotNotificationService: 企业微信机器人实现 / WeCom bot implementation
- RoutedNotificationService: 路由分发器 / Router dispatcher
...
# 通道枚举：email / sms / wechat（个人微信）（废弃，违规商用，且功能被企业微信替代）/ wechat_corp（企业微信）/ ding
